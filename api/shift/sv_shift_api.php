...
class SY_App extends SY_Framework{
    ...
    public function CALLBACK__summary_sv(){
        ...
        try{
            ...
            if($row_count == 1){
                $date_sta = $shift_section[0]['tshs_start_date'];
                $date_end = $shift_section[0]['tshs_end_date'];
            }else{
                echo json_encode("ng");
                exit();
            }
        ...
        */
        ...
        $roop_date_sta = new Datetime($date_sta);
        $roop_date_end = new Datetime($date_end);
