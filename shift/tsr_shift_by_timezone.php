...
class SY_App extends SY_Framework{
    ...
    public function CALLBACK__INDEX(){
        ...
        $today = date("Y-m-d");
        //1ヶ月後の日付を取得
        $next_month = $this->getNextDate($today);
        $next_month = date('Y-m-d', strtotime($next_month));

        $this->result->add('date_sta',$today);
        $this->result->add('date_end',$next_month);

        $this->display('shift/tsr_shift_by_timezone.tpl');
