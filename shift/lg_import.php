        ...
        if (is_uploaded_file($_FILES["lg_file_upload"]["tmp_name"])) {
            if (move_uploaded_file($_FILES["lg_file_upload"]["tmp_name"], "../excel/" . $_FILES["lg_file_upload"]["name"])) {
                ...
                try{
                    ...
                    $sql = " DELETE FROM td_shift_business WHERE tdsb_shift_date = :tdsb_shift_date";
                    $sql .= " AND tdsb_business_id IN (SELECT tmbc_business_id FROM tm_business_category WHERE tmbc_prefix != '' AND tmbc_prefix IS NOT NULL)";
                    $stmt = $this->mysql->prepare($sql);
                    $stmt->bindValue(":tdsb_shift_date" , $shift_date);
