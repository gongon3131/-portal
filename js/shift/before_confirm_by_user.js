	...
	$.ajax({
		...
		success: function(json_data) {   
            ...
            if(json_data == ""){
                ...
                paging_form();
                paging_graph();
                set_before_confirm_shift_for_graph_by_user();
                //30分設定ボタンは非表示に
...
function paging_form(){
    ...
    Object.keys(shift_data_ary).forEach(function(key) {
        ...
        contant += '<tr>';
        //日付
        contant += '<td class="hope_shift_summary_cell1"><a href="#" class="shift_by_user_fm" data-target-date-fm="' + current_date + '">' + current_date + '</a></td>';
        //第1区間
        contant += '<td>';
        contant += '<div class="form-inline">';
        contant += '<div class="form_center">';
        contant += '<input type="text" data-target-date-fm="' + current_date + '" id="bc_first_hour_sta_' + current_date + '" name="bc_first_hour_sta_' + current_date + '" value="' + first_hour_sta + '" class="form-control w15 form_right shift_form_text" oninput="value = value.replace(/[^0-9]+/i,' + "'');" + '">';
        contant += '：';
        contant += '<select data-target-date-fm="' + current_date + '" name="bc_first_min_sta_' + current_date  + '" class="form-control shift_form_text" id="bc_first_min_sta_' + current_date  + '">';
        if(first_min_sta = "00"){
        ...
        contant += '</select>';
        contant += '&nbsp;～&nbsp;';
        contant += '<input type="text" data-target-date-fm="' + current_date + '" id="bc_first_hour_end_' + current_date + '" name="bc_first_hour_end_' + current_date + '" value="' + first_hour_end + '" class="form-control w15 form_right shift_form_text">';
        contant += '：';
        contant += '<select data-target-date-fm="' + current_date + '" name="bc_first_min_end_' + current_date + '" class="form-control shift_form_text" id="bc_first_min_end_' + current_date + '">';
        if(first_min_end = "00"){
            contant += '<option label="" value="00" selected>00</option>';
            contant += '<option label="" value="30">30</option>';
        ...
        contant += '<div class="form-inline">';
        contant += '<div class="form_center">';
        contant += '<input type="text" data-target-date-fm="' + current_date + '" id="bc_second_hour_sta_' + current_date + '" name="bc_second_hour_sta_' + current_date + '" value="' + second_hour_sta + '" class="form-control w15 form_right shift_form_text">';
        contant += '：';
        contant += '<select data-target-date-fm="' + current_date + '" name="bc_second_min_sta_' + current_date + '" class="form-control shift_form_text" id="bc_second_min_sta_' + current_date + '">';
        ...
        contant += '</select>';
        contant += '&nbsp;～&nbsp;';
        contant += '<input type="text" data-target-date-fm="' + current_date + '" id="bc_second_hour_end_' + current_date + '" name="bc_second_hour_end_' + current_date + '" value="' + second_hour_end + '" class="form-control w15 form_right shift_form_text">';
        contant += '：';
        contant += '<select data-target-date-fm="' + current_date + '" name="bc_second_min_end_' + current_date + '" class="form-control shift_form_text" id="bc_second_min_end_' + current_date + '">';
        ...
        //メモ
        contant += '<td><input type="text" data-target-date-fm="' + current_date + '" name="tdbc_memo_' + current_date + '" value="' + shift_data_ary[key]['tdbc_memo'] + '" id="tdbc_memo_' + current_date + '" class="form-control shift_form_text"></td>';
        //公開・非公開フラグ
...
$('a[data-toggle="tab"]').on('show.bs.tab', function (e) {
    ...
    //現在のタブ名取得
	var tab_name = activated_tab.href.replace("http://192.168.4.233/new_portal/shift/before_confirm_by_user.php#" , "");
	//フォーム⇒グラフへのタブ遷移
...
$(document).on("click","[id^=bc_first_],[id^=bc_second_],[id^=tdbc_memo_ ]", function() {
    ...
    //公開非公開フラグ
    var confirm_flg = $("#confirm_flg_" + target_date).val();
    if(confirm_flg == 1){
...
$(document).on("click","[id^=holiday_]", function() {
    ...
    //公開非公開フラグ
    var confirm_flg = $("#confirm_flg_fm_" + target_date).val();
    if(confirm_flg == 1){
        alert("すでに確定済みのシフトなので編集できません");
        return false;
    }        
...
$(document).on("click","#bofore_confirm_regist", function() {
	...
	$.ajax({
		...
		scriptCharset: 'utf-8',
		data:          {
						'action'	  : 'before_confirm_shift_regist_by_user',
