function save_before_confirm_for_form(){
...
function save_before_confirm_for_form_by_user(){
    //console.log(shift_data_ary);
    var json = shift_data_ary;
    Object.keys(json).forEach(function(key) {
        ...
        if($("#holiday_" + current_date).prop("checked") == true){
            shift_data_ary[key]['tdbc_holiday_flg'] = 1;
        }else if($("#holiday_" + current_date).prop("checked") == false){
            shift_data_ary[key]['tdbc_holiday_flg'] = 0;
        }
        if($("#paid_holiday_" + current_date).prop("checked") == true){
            shift_data_ary[key]['tdbc_paid_holiday_flg'] = 1;
        }else if($("#paid_holiday_" + current_date).prop("checked") == false){
            shift_data_ary[key]['tdbc_paid_holiday_flg'] = 0;
        }
        if($("#midnight_" + current_date).prop("checked") == true){
            shift_data_ary[key]['tdbc_midnight_flg'] = 1;
        }else if($("#midnight_" + current_date).prop("checked") == false){
            shift_data_ary[key]['tdbc_midnight_flg'] = 0;
        }

        shift_data_ary[key]['tdbc_memo'] = $("#tdbc_memo_" + current_date).val();
        
    })//end of foreach()					
    //console.log(shift_data_ary);
}

function save_before_confirm_for_graph(){
    var json = shift_data_ary;

    Object.keys(json).forEach(function(key) {
        
        //まず、メモ欄に記述があれば保存
        shift_data_ary[json[key]['tdbc_user_id']]['tdbc_memo'] = $("#tdbc_memo_gh" + json[key]['tdbc_user_id']).val();
...
function save_before_confirm_for_graph_by_user(){
    var json = shift_data_ary;
