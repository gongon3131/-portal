...
class SY_App extends SY_Framework{
    ...
    public function CALLBACK__INDEX(){
        ...
        //1ページ表示件数取得
        $displayrows = $this->get_settings("contents_page_rows");
        if(empty($displayrows) == true){
            $displayrows = 10;
        }
        $this->result->add('displayrows',$displayrows);
        //サイドバーのカテゴリー取得
        $this->result->add('category_ary',$array_category);
        $this->result->add('c_flg',1);
        //$this->result->add('user_id',$_SESSION['login_info']['user_id']);
        $this->display('board/contents_summary.tpl');
